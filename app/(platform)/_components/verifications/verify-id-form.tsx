...
const VerifyIdentityForm = ({ apiKey }: { apiKey: string }) => {
	...
	const [imageTypeToRemove, setImageTypeToRemove] = useState<
		"face" | "frontId" | "backId" | null
	>(null);
	...
	const verifyIdentity = async () => {
		try {
			...
			const result = await verifyIdentityServerSide(uploadedImages, apiKey);
	...
	const confirmRemoveImage = () => {
		if (imageTypeToRemove) {
			...
			setUploadedImages((prev) => ({
				...prev,
				[imageTypeToRemove === "face"
					? "face_Image"
					: imageTypeToRemove === "frontId"
						? "front_id_Image"
						: "back_id_Image"]: "",
			}));
							...
							<div className="mt-8 flex flex-col space-y-4">
								<div className="flex items-center justify-center">
									...
									<Label htmlFor="confirmSelection" className="">
										...
										user data is not saved, kept private and confidential by
										Deeptrack.
									</Label>
								</div>
								<Button
